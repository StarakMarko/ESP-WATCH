...
// --- GET WEATHER (Виправлена і надійна версія) ---
bool getWeather()
{
  WiFiClient client;
  ...
  // --- ПРОПУСК ЗАГОЛОВКІВ (Найнадійніший метод) ---
  // Шукаємо кінець заголовків (порожній рядок)
  if (!client.find("\r\n\r\n"))
  {
  ...
  // --- ПАРСИНГ JSON ---
  // Фільтр: кажемо ArduinoJson брати ТІЛЬКИ ці дані, решту ігнорувати
  StaticJsonDocument<200> filter;
  filter["daily"]["weathercode"] = true;
  filter["daily"]["temperature_2m_max"] = true;
  filter["daily"]["temperature_2m_min"] = true;
  // Час не беремо з JSON, щоб економити пам'ять (використаємо RTC)

  // Виділяємо пам'ять під JSON.
  // Ваш JSON ~800 байт, але з запасом беремо 3072 для безпеки
  ...
  // Перевіряємо, чи є всередині об'єкт daily
  ...
  // Перевіряємо, чи масиви не пусті
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    // Дату генеруємо самі: сьогодні + i * 24 години
...
void wifiTask(void *parameter)
{
  if (connectToWiFi())
  {
    ...
    // 2. Weather (Тільки якщо треба)
    struct timeval tv_now;
    gettimeofday(&tv_now, NULL);
    if (!hasWeatherData || (tv_now.tv_sec - lastWeatherFetchTime > 3600))
    {
      delay(100); // Даємо стеку трохи відпочити після глюкози
...
void setup()
{
  ...
  if (wokeByButton)
  {
    ...
    while (millis() - lastInputTime < INTERACTIVE_TIMEOUT_MS)
    {
      ...
      {
        if (dataFetchedSuccessfully || weatherFetchedSuccessfully)
          updateDisplay();
        needsUpdate = false;
      }
      delay(10);
